...
Написан калькулятор‑замыкание с различными операциями, которые меняются в самом коде, и начальным значением. Внутри calc() происходит обращение к result (который равен 1) и выполняется операция деления: result /= 4 → result становится 0.25. Это значение возвращается и выводится на экран, а также сохраняется для следующих вызовов. При делении на ноль, программа выдает ошибку.
